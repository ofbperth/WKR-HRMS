...
export default async function Page({ searchParams }: { searchParams: Record<string, string | string[] | undefined> }) {
  ...
  return <AppShell user={user}><div className="space-y-6">
    <div><h1 className="text-2xl font-bold">RM Actions</h1><p className="mt-2 text-slate-600">Follow action plans, owner progress, overdue work, and verification queue.</p></div>
    <div className="grid gap-4 md:grid-cols-3"><Card><CardHeader><CardTitle>Open actions</CardTitle></CardHeader><CardContent className="text-3xl font-bold">{actions.filter((a) => a.status !== "Verified").length}</CardContent></Card><Card><CardHeader><CardTitle>Overdue</CardTitle></CardHeader><CardContent className="text-3xl font-bold text-red-700">{overdue}</CardContent></Card><Card><CardHeader><CardTitle>Waiting verification</CardTitle></CardHeader><CardContent className="text-3xl font-bold">{actions.filter((a) => a.status === "Done").length}</CardContent></Card></div>
...
function ActionTable({ actions, detailBase }: { actions: any[]; detailBase: string }) {
  return <div className="overflow-hidden rounded-xl border bg-white"><div className="overflow-auto"><table className="w-full min-w-[900px] table-fixed text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="w-20 px-3 py-3">Open</th><th className="w-32 px-3 py-3">Incident</th><th className="w-36 px-3 py-3">Unit</th><th className="px-3 py-3">Action</th><th className="w-44 px-3 py-3">Owner</th><th className="w-28 px-3 py-3">Due</th><th className="w-32 px-3 py-3">Status</th></tr></thead><tbody className="divide-y">{actions.length === 0 ? <tr><td className="px-4 py-8 text-center text-slate-500" colSpan={7}>No action plans</td></tr> : actions.map((action) => <tr key={action.id}><td className="px-3 py-3"><Link className="rounded-md border px-3 py-2 text-xs" href={`${detailBase}/${action.incidentId}`}>View</Link></td><td className="break-words px-3 py-3 font-semibold">{action.incident.incidentNo}</td><td className="break-words px-3 py-3">{action.incident.incidentUnit.name}</td><td className="px-3 py-3"><div className="line-clamp-2 break-words font-medium">{action.title}</div><div className="line-clamp-1 break-words text-xs text-slate-500">{action.incident.title}</div></td><td className="break-words px-3 py-3">{action.owner.name}</td><td className="px-3 py-3">{formatDateOnly(action.dueDate)}</td><td className="break-words px-3 py-3">{action.status}</td></tr>)}</tbody></table></div></div>;
}
