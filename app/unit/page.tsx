...
export default async function Page() {
  ...
  return <AppShell user={user}><RoleHome title="Unit Manager Workspace" description="ติดตาม incident ที่เกิดในหน่วยงานของตนเอง">
    ...
    <div className="grid gap-4 md:grid-cols-3">
      ...
      <Link href="/unit/incidents" className="rounded-xl border bg-white p-5 shadow-sm hover:bg-slate-50"><div className="font-semibold">Incident Log</div><p className="mt-1 text-sm text-slate-600">รายการ incident ของหน่วยงาน</p></Link>
    </div>
  </RoleHome></AppShell>;
}
