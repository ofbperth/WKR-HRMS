export default function Loading() {
  return <div className="space-y-6">
    ...
    <div className="overflow-hidden rounded-lg border bg-white">
      {Array.from({ length: 7 }).map((_, index) => <div key={index} className="grid grid-cols-4 gap-3 border-b p-3 last:border-b-0">
        {Array.from({ length: 4 }).map((__, cell) => <div key={cell} className="h-6 animate-pulse rounded bg-slate-100" />)}
      </div>)}
    </div>
  </div>;
}
