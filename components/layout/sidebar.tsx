...
const rmTeamMenu: MenuSection[] = [
  {
    ...
    items: [
      ...
      { href: "/rm/risks", label: "Risk Register", icon: <ShieldAlert size={18} /> },
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/rm/risks/proposals", label: "Risk Proposals", icon: <ShieldAlert size={18} /> },
    ],
  },
  {
    title: "Analytics",
  ...
  },
  {
    title: "ตั้งค่า",
    items: [
      { href: "/rm/automation", label: "งานอัตโนมัติ", icon: <Settings size={18} /> },
...
const adminMenu: MenuSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/executive/risks", label: "Top Hospital Risks", icon: <ShieldAlert size={18} /> },
  ...
  UnitManager: [
    {
      ...
      items: [
        ...
        { href: "/unit/risks", label: "Risk Register", icon: <ShieldAlert size={18} /> },
      ],
    },
    {
      title: "ติดตามงาน",
      items: [
        ...
        { href: "/unit/risks/proposals", label: "Risk Proposals", icon: <ShieldAlert size={18} /> },
        { href: "/executive/monthly-report", label: "รายงานสรุป", icon: <FileText size={18} /> },
        { href: "/my-reports", label: "รายงานของฉัน", icon: <FileText size={18} /> },
        { href: "/profile", label: "ข้อมูลส่วนตัว", icon: <UserCircle size={18} /> },
      ],
      ...
      <aside className="fixed inset-y-0 left-0 hidden w-72 flex-col border-r border-emerald-100 bg-white/95 shadow-xl shadow-emerald-950/5 backdrop-blur print:hidden lg:flex">
        <div className="shrink-0 p-5 pb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-lg font-bold tracking-normal text-emerald-700">WKR-HRMS</div>
              <div className="text-xs text-muted-foreground">ระบบรายงานและบริหารความเสี่ยงโรงพยาบาล</div>
            </div>
          </div>
        </div>
          ...
          {sections.map((section) => (
            <div key={section.title} className="space-y-1">
              ...
              {section.items.map((item) => (
                <Link
                  key={item.href}
                  className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium leading-5 text-slate-700 transition hover:bg-emerald-50 hover:text-emerald-800"
                  href={item.href}
                >
                  <span className="shrink-0 text-emerald-600">{item.icon}</span>
        ...
        <div className="shrink-0 border-t border-emerald-100 bg-white/90 p-3">
          <div className="rounded-lg border border-emerald-100 bg-emerald-50/70 p-4 text-sm">
            <div className="font-semibold">{user.name}</div>
            <div className="mb-2 truncate text-xs text-muted-foreground">{user.email}</div>
            <RoleBadge role={user.role} />
            <div className="mt-1 text-xs text-slate-500">{roleLabels[user.role]}</div>
          </div>
        </div>
      </aside>
      <main className="print:pl-0 lg:pl-72">
        <header className="sticky top-0 z-10 flex h-16 items-center justify-between gap-3 border-b border-emerald-100 bg-white/86 px-4 backdrop-blur print:hidden lg:px-8">
          ...
          <div className="flex shrink-0 items-center gap-3">
            ...
            <form action="/api/auth/logout" method="post">
              <button className="flex items-center gap-2 rounded-lg border border-emerald-100 bg-white px-3 py-2 text-sm font-medium text-slate-700 shadow-sm hover:bg-emerald-50">
                <LogOut size={16} />
        ...
        <nav className="flex gap-2 overflow-x-auto border-b border-emerald-100 bg-white/95 px-4 py-3 print:hidden lg:hidden">
          {mobileItems.map((item) => (
            <Link
              key={item.href}
              className="inline-flex min-h-10 max-w-[13rem] shrink-0 items-center gap-2 rounded-lg border border-emerald-100 bg-white px-3 py-2 text-xs font-semibold leading-4 text-slate-700 shadow-sm"
              href={item.href}
            >
              <span className="shrink-0 text-emerald-600">{item.icon}</span>
