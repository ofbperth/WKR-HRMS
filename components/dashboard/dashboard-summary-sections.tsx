...
import { getRiskDashboardWidget } from "@/lib/risk-register";
...
export async function UnitDashboardSummary({ searchParams, unitId }: { searchParams: DashboardSearchParams; unitId: string | null }) {
  const started = Date.now();
  const [data, riskWidget] = await Promise.all([
    getDashboardSummary({ ...normalizeDashboardSearchParams(searchParams), scopeUnitId: unitId }),
    getRiskDashboardWidget(unitId),
  ]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="dashboard-stat-grid">
        ...
        <LinkedStatCard title="My Risk Status: High/Extreme" value={riskWidget.highOrExtreme} href="/unit/risks" />
        <LinkedStatCard title="My Risk Status: Review Due" value={riskWidget.dueSoon} href="/unit/risks?dueReview=30d" />
        <LinkedStatCard title="My Risk Status: Overdue" value={riskWidget.overdue} href="/unit/risks?dueReview=overdue" />
        <LinkedStatCard title="My Risk Status: Open Actions" value={riskWidget.openActions} href="/unit/risks" />
      </div>
