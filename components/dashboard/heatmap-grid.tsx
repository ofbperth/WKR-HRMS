...
export function HeatmapGrid({ data }: { data: { units: Array<{ id: string; name: string }>; yMode: string; rows: Array<{ row: string; cells: any[] }> } }) {
  ...
  return <div className="space-y-4">
    ...
    <div className="overflow-hidden rounded-lg border bg-white"><div className="overflow-auto"><table className="w-full min-w-[960px] text-sm"><thead className="bg-slate-50"><tr><th className="sticky left-0 bg-slate-50 px-4 py-3 text-left">{data.yMode === "simpleCategory" ? "หมวด SIMPLE" : "ระดับความรุนแรง"}</th>{data.units.map(unit => <th key={unit.id} className="px-3 py-3 text-center">{unit.name}</th>)}</tr></thead><tbody>{data.rows.map(row => <tr key={row.row} className="border-t"><td className="sticky left-0 bg-white px-4 py-3 font-medium">{row.row}</td>{row.cells.map(cell => <td key={`${cell.unitId}-${row.row}`} className="p-1 text-center"><Link href={`/rm/search?unitId=${cell.unitId}&${data.yMode === "simpleCategory" ? `simpleCategory=${encodeURIComponent(row.row)}` : `severity=${row.row}`}`} title={`หน่วยงาน: ${cell.unit}\n${data.yMode === "simpleCategory" ? "หมวด SIMPLE" : "ระดับความรุนแรง"}: ${row.row}\nจำนวน incident: ${cell.count}\nคะแนนถ่วงน้ำหนัก: ${cell.score}\nระดับสูงสุด: ${cell.highestSeverity}\nRCA ที่เปิดอยู่: ${cell.openRca}\nแผนแก้ไขเกินกำหนด: ${cell.overdueActions}`} className={`block rounded-md px-3 py-2 font-semibold ${color(metric === "count" ? cell.count : cell.score, max)}`}>{metric === "count" ? cell.count : cell.score}</Link></td>)}</tr>)}</tbody></table></div></div>
  </div>;
}
