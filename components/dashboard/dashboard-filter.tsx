...
export function DashboardFilter({ units = [], categories = [], showUnit = true }: { units?: Array<{ id: string; name: string }>; categories?: string[]; showUnit?: boolean }) {
  ...
  function buildFilterHref(formData: FormData) {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const href = buildFilterHref(formData);
    startTransition(() => router.push(href));
  }

  return <form onSubmit={submit} aria-busy={isPending} className="dashboard-filter-form grid grid-cols-1 gap-3 rounded-lg border bg-white p-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
    ...
    <div className="dashboard-filter-actions flex flex-wrap items-center justify-end gap-4">
      {isPending ? <span className="text-xs font-medium text-emerald-700">Applying...</span> : null}
